...
public class MyApplet2 extends Applet implements Runnable {
    ...
    public boolean keyDown(Event evtObj, int key) {
        switch (key) {
            ...
            case Event.ENTER:
                ...
                break;
            case Event.DOWN:
                rect.moveDown(getGraphics(), RECT_D, getBounds().height);
                break;
            case Event.UP:
                rect.moveUp(getGraphics(), RECT_D, 0);
                break;
            case Event.RIGHT:
                rect.moveRight(getGraphics(), RECT_D, getBounds().width);
                break;
            case Event.LEFT:
                rect.moveLeft(getGraphics(), RECT_D, 0);
                break;
        }
    ...
    public void start() {
        ...
        rect = new Rect2(getBounds().width / 2 - RECT_SIZE / 2, getBounds().height / 2 + RECT_SIZE / 2, RECT_SIZE, RECT_SIZE, new Color(55, 55, 55));
